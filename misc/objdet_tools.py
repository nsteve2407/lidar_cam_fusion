...
def project_detections_into_bev(bev_map, detections, configs, color=[]):
    for row in detections:
        ...
        # bev_corners = np.where(bev_corners>=0,bev_corners,0)
        # bev_corners = np.where(bev_corners<608,bev_corners,608)
        
        # print('BEV:', bev_corners)
        # draw object as box
        corners_int = bev_corners.reshape(-1, 1, 2).astype(int)
        cv2.polylines(bev_map, [corners_int], True, color, 2)
        # print('Corners before',corners_int)

        # draw colored line to identify object front
        corners_int = bev_corners.reshape(-1, 2).astype(int)
        # print('Corners after',corners_int)
