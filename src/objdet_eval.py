...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            fl,rl,rr,fr = tools.compute_box_corners(bbox.center_x,bbox.center_y,bbox.width,bbox.length,bbox.heading)
            
            ## step 2 : loop over all detected objects
            for det in detections:

            ## step 3 : extract the four corners of the current detection _score,_x,_y,_z,_h,_w,_l,_yaw]
                dfl,drl,drr,dfr = tools.compute_box_corners(det[1],det[2],det[5],det[6],det[7])
            
            ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dx,dy,dz = bbox.center_x-det[1],bbox.center_y-det[2],bbox.center_z-det[3]
                # print('Labels:', bbox.center_x,bbox.center_y,bbox.width,bbox.length,bbox.heading)
            
                # print('Detections:',det[1],det[2],det[5],det[6],det[7])
            ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                label_box = Polygon((fl,rl,rr,fr))
                det_box = Polygon((dfl,drl,drr,dfr))
                iou = label_box.intersection(det_box).area/label_box.union(det_box).area
