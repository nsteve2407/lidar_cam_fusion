...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        x_ = np.ones((4,1))
        x_[:3,:] = x[:3,:]
        x_sens = self.veh_to_sens@x_
        phi = np.arctan(x_sens[1]/x_sens[0])       
            
        if phi<self.fov[0] or phi>self.fov[1]: return False
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # try:
            # print('State : ',x)
            # print('TF Matrix:',self.veh_to_sens)
            x_ = np.ones((4,1))
            x_[:3,:] = x[:3,:]
            x_sens = self.veh_to_sens@x_
            # if x_sens[0] == 0.0 : raise ZeroDivisionError
            if x_sens[0] == 0.0 : x_sens[0]=0.0001
            x1 = self.c_i - (self.f_i*x_sens[1])/x_sens[0]
            x2 = self.c_j - (self.f_j*x_sens[2])/x_sens[0]
            # print(x1[0,0],x2[0,0])

            return np.matrix([[x1[0,0]],[x2[0,0]]])
