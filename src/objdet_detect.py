...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.pretrained_path = os.path.join(parent_path,'tools','objdet_models','resnet','pretrained','fpn_resnet_18_epoch_300.pth')
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            detections_ = decode(outputs['hm_cen'], outputs['cen_offset'], outputs['direction'], outputs['z_coor'],
                                outputs['dim'], K=configs.K)
            
            detections_ = detections_.cpu().numpy().astype(np.float32)
            detections_ = post_processing(detections_, configs)
            detections_ = detections_[0]
            detections = []
            if len(detections_[1])>0:
                for det in detections_[1]:
                    _score, _x, _y, _z, _h, _w, _l, _yaw = det
                    detections.append([_score,_x,_y,_z,_h,_w,_l,_yaw])
            #######
            ####### ID_S3_EX1-5 END #######     
    ...
    ## step 2 : loop over all detections
        ...
        ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure
            _score, _x, _y, _z, _h, _w, _l, _yaw = detections[i]
            unit_s = (configs.lim_x[1]-configs.lim_x[0])/configs.bev_height
            x = configs.lim_x[0] + (_y*unit_s)
            y = configs.lim_y[0] + (_x*unit_s)
            w = _w*unit_s
            l = _l*unit_s
